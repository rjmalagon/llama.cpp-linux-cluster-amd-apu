...
RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential cmake git python3 python3-pip libssl-dev libgomp1 \
    curl ca-certificates ccache make wget \
    && rm -rf /var/lib/apt/lists/*

WORKDIR /build

# ── TheRock ROCm builder base (replaces rocm/dev-ubuntu-24.04:7.2.4-complete) ──

FROM ubuntu:24.04 AS therock-rocm
...
# ── Vulkan builder base (also THEROCK-based) ──

FROM therock-rocm AS builder-base-vulkan
...
COPY llama-swap/docker/unified/install-whisper.sh /build/
...
COPY llama-swap/docker/unified/install-ik-llama.sh /build/
...
# ── TheRock ROCm runtime base (replaces rocm/dev-ubuntu-24.04:7.2.3-complete) ──

FROM therock-rocm AS runtime-vulkan
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    python3 curl ca-certificates libibverbs1 curl ffmpeg \
    && rm -rf /var/lib/apt/lists/*
